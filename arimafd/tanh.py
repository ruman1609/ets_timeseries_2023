...
def projection(w,circle=1.01):
    ...
    #     print('f(x) =',whole.expand())
        p = poly(whole, x)
        try:
            res = np.array(p.all_coeffs()).astype(float)
            return res
        except:
            return np.array(p.all_coeffs()).astype(complex)
...
class Anomaly_detection:
    ...
    def proc_tensor(self,window=100,metric3_no=1,window_insensitivity=100, metric_index=3):
        """
        ...
        window_insensitivity : int (default=100)
            After the new detected changepoint,
...
#         if len(self.data.iloc[:,i].unique())/len(self.data.iloc[:,i]) > 0.99:
        if metric_index == 1:
            metric = np.sqrt(np.square(df).sum(axis=1))
            self.metric = metric
            thres = metric.mean() + 3*metric.std()
            self.ucl = thres
            self.lcl = thres
            bin_metric = (metric > thres).astype(int)
    ...
    #             print('Metric1')
            ...
            a_t = lambda t : np.exp((2 * t) / (40.5 * window))
            metric2 = metric1.copy()
            for t in range(metric1.shape[0]):
                windows = metric1.iloc[t:t+window].reset_index(drop=True)
                a_i = pd.Series([a_t(t+i) for i in range(windows.shape[0])])
                metric2.iloc[t] = np.sum(windows * a_i)
            
            s_metric2 = metric2.sort_values()
            index = (9 * metric2.shape[0]) / 10
            q_value = s_metric2[int(index + 1)]
            metric3 = metric2.copy()
            for i in range(metric3.shape[0]):
                metric3.iloc[i] = 0 if metric2.iloc[i] < q_value else metric2.iloc[i]
            
            max_idx = argrelextrema(np.array(metric3), np.greater)
            
            bin_metric = metric3.copy()
            bin_metric[:] = float(0)
            bin_metric.iloc[max_idx[0]] = float(1)
            bin_metric = bin_metric.astype(int)
